...
from PPO.model import RGBNet,DepthNet,MptNet
...
class Net(nn.Module):
     def __init__(self, out_dim):
          ...
          self.model_rgb = RGBNet().to(device0)
          self.model_depth = DepthNet().to(device0)
          self.model_mpt = MptNet().to(device0)
          self.fuse_rgbdepth = Fusion_rgbdepth().to(device0)
          self.fuse_rgbmpt = Fusion_rgbmpt().to(device0)
          self.fuse_depthmpt = Fusion_depthmpt().to(device0)
          self.triplet_attention = TripletAttention_new().to(device0)
